...
def evaluate_windows(windows: dict[tuple[int, BehaviourType], list[float]]) -> list[TriggeredSeat]:
    ...
    for (seat_number, signal), window in windows.items():
        if len(window) < BUFFER_WINDOW_FRAMES:
            continue
        over_threshold = sum(1 for value in window if value >= SIGNAL_THRESHOLDS[signal])
    ...
    return [
        TriggeredSeat(
            ...
            per_signal=signals,
            composite_score=max(signals.values()),
        )
...
async def process_frame(session_id: str, frame_index: int, seats: list[SeatSignals]) -> list[TriggeredSeat]:
    windows = await record_frame(session_id, frame_index, seats)
    return await claim_cooldowns(session_id, evaluate_windows(windows))
